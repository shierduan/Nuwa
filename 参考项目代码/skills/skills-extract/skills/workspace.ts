...
function loadSkillEntries(
  ...
  const skillEntries: SkillEntry[] = Array.from(merged.values()).map((skill) => {
    ...
    if (metadata?.card) {
      card = {
        name: skill.name,
        title: metadata.card.title || skill.name,
        description: metadata.card.description || skill.description,
        emoji: metadata.emoji,
        icon: metadata.card.icon,
        category: metadata.card.category,
        tags: metadata.card.tags,
        color: metadata.card.color,
        backgroundColor: metadata.card.backgroundColor,
      };
    } else {
      // Fallback card for skills without card metadata
      card = {
        name: skill.name,
        title: skill.name,
...
export function buildWorkspaceSkillCards(
  workspaceDir: string,
  opts?: {
    config?: OpenClawConfig;
    managedSkillsDir?: string;
  ...
  return eligible.map(entry => entry.card!).filter(Boolean);
}

export function buildWorkspaceSkillCardsByCategory(
  workspaceDir: string,
  opts?: {
    config?: OpenClawConfig;
    managedSkillsDir?: string;
  ...
  const cards = buildWorkspaceSkillCards(workspaceDir, opts);
  const categories: Record<string, SkillCard[]> = {};
  ...
  // Sort categories and cards within each category
  ...
  for (const entry of userInvocable) {
    ...
    const dispatch = (() => {
      ...
      return { kind: "tool", toolName, argMode: "raw" } as const;
          ...
          // Include card metadata in command spec
          const cardMetadata = entry.card ? {
            ...
            backgroundColor: entry.card.backgroundColor,
          } : undefined;
