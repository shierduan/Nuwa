...
export function resolveOpenClawMetadata(
  ...
  // Parse skill card fields
  let card;
  if (metadataObj.card && typeof metadataObj.card === "object") {
    card = {
      title: typeof metadataObj.card.title === "string" ? metadataObj.card.title : undefined,
      description: typeof metadataObj.card.description === "string" ? metadataObj.card.description : undefined,
      icon: typeof metadataObj.card.icon === "string" ? metadataObj.card.icon : undefined,
      category: typeof metadataObj.card.category === "string" ? metadataObj.card.category : undefined,
      tags: Array.isArray(metadataObj.card.tags) ? metadataObj.card.tags.map(tag => typeof tag === "string" ? tag.trim() : "").filter(Boolean) : undefined,
      color: typeof metadataObj.card.color === "string" ? metadataObj.card.color : undefined,
      backgroundColor: typeof metadataObj.card.backgroundColor === "string" ? metadataObj.card.backgroundColor : undefined,
    };
    // Remove undefined fields
    Object.keys(card).forEach(key => {
      if (card[key as keyof typeof card] === undefined) {
        delete card[key as keyof typeof card];
      }
    });
    // Only include card object if it has at least one field
